...
| E | PW管理（確認・変更 + 再設定） | login, F-1, E-1〜E-3 | P0 |
...
### 4.5 フローE: PW管理（確認・変更 + 再設定）

> ログイン → PW確認・変更（F-1） → パスワード再設定（E-1〜E-3）を統合したフロー。
> タブ順: ログイン →「パスワードを忘れた」→ E-1 メール入力 → E-2 認証 → E-3 新PW

#### PW確認・変更（F-1: オーナー/店長のみ）
...
│           └── password_resets_controller.rb  # E: PW管理（再設定） + F: 管理者PW再発行
├── models/
│   ├── invitation.rb
