...
| F | PW確認・変更（ログイン後） | F-1 | P0 |
| G | 管理者によるPW再発行 | B-1, D-1, D-2 | P0 |
| H | 招待メール再送信 | B-1, C-1, D-1, D-2 | P0 |
| I | ログイン画面改修 | login | P1 |
...
### 4.8 フローF: PW確認・変更（オーナー/店長）

> ログイン後、ダッシュボードから自分のPWを確認・変更する機能。
...
│           └── password_resets_controller.rb  # E: PW再設定 + G: 管理者PW再発行
├── models/
│   ├── invitation.rb
...
5. フローE: PW管理（PW確認・変更 + パスワード再設定）
6. フローF: PW確認・変更（ログイン後）
7. フローG: 管理者によるPW再発行
8. フローH: 招待メール再送信
