...
5. `manager` の場合、`stores.manager_id` を更新
...
3. `users` レコード削除
4. 関連する `stores.manager_id` をNULLに更新
5. 一覧から消える
...
│   └── api/
│       └── v1/
│           ├── stores_controller.rb          # 拡張: manager情報返却
...
### Phase 1（P0 — 最優先）
1. DBマイグレーション（invitations, users.clerk_user_id/role, stores.manager_id）
