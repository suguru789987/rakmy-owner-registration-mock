...
> ⚠ **本要件書は 2026-04-08 時点の v1.5 です。以降の重要決定（衝突回避設計・管理者登録2方式・招待メール再リリース・ハードモーダル統一・QA19件回答 等）はメイン仕様書側で更新されています。**
>
> **実装時の参照優先順位**:
> 1. **入口**: [`HANDOFF.md`](./HANDOFF.md)
> 2. **メイン仕様書**: [`20260501_新規登録運用登録分離仕様.md`](./20260501_新規登録運用登録分離仕様.md)（現行コア §1〜§9 / §16 / §20 / §23 / §24〜§27）
> 3. **本書（要件書）** — DB設計 / API設計 / バックエンド実装観点の参考（`rakmy/rakmy_server` 側の実装計画用）。UI挙動・状態判定・条件分岐は上記2点が優先。

> **バージョン:** v1.5
> **作成日:** 2026-03-02
> **最終更新:** 2026-04-08（本文最終更新日 / 注記追加 2026-06-04）
  ...
  "employees": [
    {
      ...
      "email": "nishikata+test2@example.com",
      "phone": "090-1234-1234",
      "status": "active",
      "store": { "id": 1, "name": "渋谷店" }
    }
...
| 交通費アラート | 所属店舗を変更すると「交通費（日額）を新しい店舗に合わせて更新してください」の警告を表示。同時に交通費単位・交通費/単位の入力欄を赤枠（`#c62828`）+薄赤背景（`#fff5f5`）でハイライト。元の店舗に戻すとリセット |
...
> **注意:** M-2（オーナー招待）、M-3（管理者招待）、M-5（ウェルカムメール）は廃止。管理者は直接登録のためメール招待なし。サインアップ完了通知はC-5/PW設定完了画面で代替。
...
| 従業員編集モーダル（オーナー・有効） | メールアドレス変更不可（disabled）。名前・電話・雇用区分・交通費・所属店舗（全店舗）を編集可能。所属店舗変更時は交通費更新アラート＋交通費欄を赤枠ハイライト |
| 従業員編集モーダル（オーナー・招待中/期限切れ） | メールアドレス変更可能。変更時は旧招待リンク無効化＋新招待メール自動再送の注意書きを表示。所属店舗は全店舗から選択可能。所属店舗変更時は交通費更新アラート＋交通費欄を赤枠ハイライト |
| 従業員編集モーダル（管理者・有効） | オーナー版と同様。所属店舗は担当店舗のみのセレクトボックス（担当店舗間で異動可能）。所属店舗変更時は交通費更新アラート＋交通費欄を赤枠ハイライト |
| 従業員編集モーダル（管理者・招待中/期限切れ） | オーナー版と同様。所属店舗は担当店舗のみのセレクトボックス。所属店舗変更時は交通費更新アラート＋交通費欄を赤枠ハイライト |
| 従業員登録モーダル（オーナー） | 上部に「所属店舗」セレクトボックス追加（全店舗から選択可能）。登録時に `user_store_assignments` レコードを作成 |
| 従業員登録モーダル（管理者） | 担当店舗が1つの場合は初期選択済み＋disabled、複数の場合は担当店舗のみに絞ったセレクトボックス。登録時に `user_store_assignments` レコードを作成 |
