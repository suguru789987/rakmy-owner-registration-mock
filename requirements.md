...
ALTER TABLE users
  ...
  ADD COLUMN store_id BIGINT UNSIGNED,           -- 従業員の所属店舗（ホーム店舗）
  ADD COLUMN invitation_id BIGINT UNSIGNED,
  ADD FOREIGN KEY (store_id) REFERENCES stores(id);
```

> **所属店舗と打刻店舗の区別:** `users.store_id` は従業員の所属店舗（ホーム店舗）を表す。打刻レコードの店舗とは別概念。将来的に親システム（ラクミー本体）でヘルプ勤務識別（所属店舗 ≠ 打刻店舗）を実装する際の基盤となる。
...
| DB更新 | `users.store_id` を新しい店舗IDに更新 |
| 給与計算 | 打刻データベースで計算。異動前の打刻は旧店舗、異動後の打刻は新店舗に計上（按分不要） |
| ヘルプ勤務との区別 | `users.store_id`（所属店舗）と打刻店舗が異なる場合、将来的にヘルプ勤務として識別可能にする（親システム連携で対応予定） |
