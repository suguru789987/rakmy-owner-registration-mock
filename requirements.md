...
従業員一覧に表示されるステータスは、招待の状態とユーザーアカウントの状態を統合して1つのバッジとして表示する。全6種類。
...
| 項目 | 内容 |
|---|---|
| いつなるか | オーナー/店長が従業員を登録した直後（招待メールが自動送信される） |
| DBの状態 | `invitations.status = pending`、`users` レコードは未作成 |
| 招待リンク | 有効（72時間以内） |
| 次の遷移先 | → 有効（PW設定完了）/ → 期限切れ（72時間経過）/ → キャンセル（管理者が取消） |
...
| 次の遷移先 | → 無効（管理者がアカウントを無効化）/ → 退会済み（退会処理） |
...
| 次の遷移先 | → 招待中（管理者が「再送」で再招待 → 新トークン発行）/ → キャンセル（管理者が取消） |

#### キャンセル

| 項目 | 内容 |
|---|---|
| いつなるか | 管理者が招待中/期限切れの従業員の「削除」ボタンを押した（= 招待取り消し） |
| DBの状態 | `invitations.status = cancelled`、`users` レコードは未作成 |
| 招待リンク | 無効 |
| 次の遷移先 | → 招待中（管理者が「再招待」で新規招待を作成）/ → 完全削除（管理者が「削除」で招待履歴を削除） |

#### 無効

| 項目 | 内容 |
|---|---|
| いつなるか | 管理者が有効なアカウントを無効化した（ログイン停止） |
| DBの状態 | `invitations.status = accepted`、`users` レコードあり（ログイン不可） |
| 次の遷移先 | → 有効（管理者が無効化を解除） |
...
| 項目 | 内容 |
|---|---|
| いつなるか | 従業員が退会した、または管理者が退会処理を行った |
| DBの状態 | `users` レコードあり（論理削除済み、ログイン不可） |
| 次の遷移先 | → 完全削除（管理者が「削除」でデータを完全削除） |
...
| キャンセル | グレー | 背景 `#f5f5f5` / 文字 `#999` |
| 無効 | 赤 | 背景 `#fce4ec` / 文字 `#c62828` |
| 退会済み | グレー | 背景 `#f5f5f5` / 文字 `#999` |

### 2.5.4 ステータス別の操作列（PW操作・編集・削除）

| ステータス | PW操作列 | 編集ボタン | 削除ボタン |
|---|---|---|---|
| 招待中 | 「再送」ボタン（アウトラインスタイル） | 有効 → 招待中モーダル（メール変更可、変更時に招待再送） | 有効 → 確認「○○への招待を取り消しますか？」→ `invitations.status = cancelled` |
| 有効 | 「PW再発行」ボタン | 有効 → 有効モーダル（メール変更不可） | 有効 → 確認「○○を削除しますか？この操作は取り消せません。」→ アカウント削除 |
| 期限切れ | 「再送」ボタン（アウトラインスタイル） | 有効 → 招待中モーダル（メール変更可、変更時に招待再送） | 有効 → 確認「○○への招待を取り消しますか？」→ `invitations.status = cancelled` |
| キャンセル | 「再招待」ボタン | 非活性（disabled, opacity: 0.4） | 有効 → 確認「○○の招待履歴を削除しますか？」→ レコード物理削除 |
| 無効 | —（ダッシュ表示） | 非活性（disabled, opacity: 0.4） | 非活性（disabled, opacity: 0.4） |
| 退会済み | —（ダッシュ表示） | 非活性（disabled, opacity: 0.4） | 有効 → 確認「○○の退会済みデータを削除しますか？」→ レコード物理削除 |
...
**エラー:**
- `404` トークン無効（キャンセル済み含む）
- `410` 有効期限切れ
- `409` 既に承諾済み

**エラー画面（フロントエンド）:**
上記エラーレスポンスを受け取った場合、専用のエラー画面を表示する。

- 表示メッセージ: 「招待リンクが無効です」
- 理由の候補を表示: 「招待が取り消された」「有効期限（72時間）が切れた」「既にアカウントが作成済み」
- 対処法: 「管理者にお問い合わせのうえ、新しい招待メールの送信を依頼してください。」
- 「ログイン画面に戻る」ボタンを配置
- モック: ナビバー「エラー > 無効な招待」タブで確認可能
...
| 承諾済み（有効） | 既存の従業員編集処理（情報の上書き更新）。**メールアドレスは変更不可** | ログイン情報のため |
| 期限切れ | 招待中と同じ（編集＋自動再送） | 新トークン・新期限で再発行 |
| キャンセル | 編集不可（再招待ボタンで新規招待を作成） | — |
| 無効・退会済み | 編集不可 | — |
...
| ステータス | 動作 | 確認ダイアログ |
|---|---|---|
| 招待中 | 招待取り消し（`invitations.status → cancelled`） | 「○○への招待を取り消しますか？」 |
| 承諾済み（有効） | アカウント削除（既存の従業員削除処理） | 「○○を削除しますか？この操作は取り消せません。」 |
| 期限切れ | 招待取り消し（`invitations.status → cancelled`） | 「○○への招待を取り消しますか？」 |
| キャンセル | レコード削除（招待履歴から完全削除） | 「○○の招待履歴を削除しますか？」 |
| 無効 | アカウント無効化解除 or 削除の選択肢を提示 | — |
| 退会済み | レコード削除 | 「○○の退会済みデータを削除しますか？」 |
...
**エラー:**
- `403` 権限不足（店長が他店舗の従業員を編集しようとした場合）
- `422` 編集不可ステータス（キャンセル・無効・退会済み）
- `422` 承諾済みユーザーのメールアドレス変更
...
**招待中・期限切れの場合:**
1. 権限チェック
2. `invitations.status` を `cancelled` に更新（招待取り消し）
3. 招待トークンを無効化

**承諾済み（有効）の場合:**
1. 権限チェック
2. 既存の従業員削除処理（`users` レコード削除 or 論理削除）
3. 関連する招待レコードのクリーンアップ

**キャンセルの場合:**
1. 権限チェック
2. `invitations` レコードを物理削除（招待履歴から完全削除）

**退会済みの場合:**
1. 権限チェック
2. `users` レコードを物理削除（退会済みデータの完全削除）

**エラー:**
- `403` 権限不足
- `422` 無効ステータスのユーザーは削除不可
...
> 未ログイン状態でのパスワード再設定フロー。
> 遷移: ログイン →「パスワードを忘れた」→ E-1 メール入力 → E-2 認証 → E-3 新PW設定 → ログイン
...
> **注意:** 管理者（オーナー/店長）自身のPW再設定は、ログイン画面の「パスワードを忘れた」（フローE）から行う。本フローは従業員のPW再発行のみを対象とする。

#### `POST /api/v1/users/:id/reset_password`
管理者が従業員のパスワードをリセットし、一時パスワードをメール通知。

**認証:** 必須（オーナー: 全店舗の従業員 / 店長: 担当店舗の従業員のみ）
...
**処理内容:**
1. 権限チェック（オーナー: 同一company_idの従業員のみ / 店長: 担当store_idの従業員のみ）
...
> 既存のアカウント設定画面。全ユーザーがサイドバー「アカウント設定」からアクセス。
...
| 従業員一覧 オーナー視点 (`/dashboard/employees`) | 店舗フィルタータブ + 所属店舗列追加 + 全6ステータス（有効/招待中/期限切れ/キャンセル/無効/退会済み）の行を表示 + ステータス別PW操作・編集・削除ボタンの出し分け |
| 従業員一覧 管理者視点 (`/dashboard/employees`) | 担当店舗の従業員のみ表示 + 全6ステータスの行を表示 + オーナー視点と同じステータス別ボタン出し分け |
| 従業員編集モーダル（有効） | メールアドレス変更不可（disabled）。名前・電話・雇用区分・交通費・所属店舗を編集可能 |
| 従業員編集モーダル（招待中/期限切れ） | メールアドレス変更可能。変更時は旧招待リンク無効化＋新招待メール自動再送の注意書きを表示 |
| 従業員登録モーダル | オーナー向け: 上部に「所属店舗」セレクトボックス追加 |
| 管理者一覧 (`/dashboard/administrators`) | 一覧テーブル + 「管理者を登録」ボタン + 編集・削除ボタン（テキストボタン）。**PW操作列なし**（管理者は自分でPW再設定可能） |
...
| 要素 | ルール |
|------|--------|
| 一覧テーブルの操作ボタン | テキストボタン（「編集」「削除」）で統一。アイコンボタンは使用しない |
| 削除ボタンの色 | 赤文字（`#c62828`） |
| 確認ダイアログ | 削除・PW再発行など不可逆操作は `confirm()` で確認 |
| 編集不可ステータス | キャンセル・無効・退会済みは編集ボタンを `disabled`（opacity: 0.4）にする |
| 削除不可ステータス | 無効は削除ボタンも `disabled`（opacity: 0.4）にする |
| 従業員一覧のステータス表示 | オーナー視点・管理者視点ともに全6ステータスの行を表示し、ボタンの出し分けを統一する |
