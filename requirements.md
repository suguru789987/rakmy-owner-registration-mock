...
CREATE TABLE invitations (
  ...
  store_id BIGINT UNSIGNED,               -- 管理者/従業員の場合の所属店舗
...
3. `users` レコード作成（role, パスワードbcryptハッシュ化）
4. `manager` の場合、`stores.manager_id` を更新
5. 初回ログイン時にパスワード変更を強制するフラグ設定（`force_password_change = true`）

> **パスワード共有:** オーナーが初期パスワードを口頭・チャット等で管理者に伝える。管理者は初回ログイン後、パスワード変更を促される。パスワードを忘れた場合は、ログイン画面の「パスワードを忘れた」（フローE）から自分でリセットする。
...
2. `users` レコード削除
3. 関連する `stores.manager_id` をNULLに更新
4. 一覧から消える
...
> **注意:** 管理者の店舗割り当ては管理者一覧（フローB）の登録・編集で行う。店舗追加時の管理者同時設定は行わない。
...
> **TODO: Clerk対応** — 現在のタイムレコードはClerkを認証基盤として使用している。本仕様書のログイン・パスワード管理・セッション管理の記述はClerk前提になっていないため、Clerkの仕様に合わせた調整が必要。具体的には、サインアップ（フローA）、PW再設定（フローE）、セッション/JWT発行の各フローをClerk APIに置き換える検討を行う。
...
|------|------|
| パスワード最小長 | 8文字 |
| パスワードハッシュ | bcrypt（has_secure_password）※Clerk移行後はClerk管理 |
| セッション管理 | 既存の仕組みを踏襲 ※Clerk移行後はClerkセッション |
...
| 店舗情報 (`/dashboard/stores`) | テーブルに「管理者」列追加（管理者一覧で登録した管理者の担当店舗を表示） |
